import { GraphComponent, IEdge, ILabelModelParameter, IModelItem, INode, Point, SimpleLabel, Size } from "yfiles";
...
export default class HTMLPopupSupport {
	...
	private labelModelParameter: any;
	private $div: any;
	...
	registerListeners() {
		...
		this.graphComponent.graph.addNodeLayoutChangedListener((node, oldLayout) => {
			if (((this.currentItem && this.currentItem === node) || IEdge.isInstance(this.currentItem)) && (node === this.currentItem.sourcePort.owner || node === this.currentItem.targetPort.owner)) {
				this.dirty = true;
			}
		});
	...
	updateLocation() {
		...
		if (this.labelModelParameter.supports(dummyLabel)) {
			...
			const newLayout = this.labelModelParameter.model.getGeometry(dummyLabel, this.labelModelParameter);
